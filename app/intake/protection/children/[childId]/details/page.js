...
export default function ProtectionChildDetailsPage() {
  ...
  useEffect(() => {
    ...
    if (!activeProfile || activeProfile.relationship !== "child") {
      router.replace(
        profileReturnAppId
          ? `/intake/protection/profile?applicationId=${encodeURIComponent(profileReturnAppId)}`
          : "/intake/protection/profile"
      );
    }
