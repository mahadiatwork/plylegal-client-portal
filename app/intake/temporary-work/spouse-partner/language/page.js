...
export default function LanguagePage() {
  ...
  useEffect(() => {
    ...
    const savedData = isSpouseProfile && profileId
      ? draft.profiles_data?.[profileId]?.language || {}
      : draft.temporary_work_spouse_language || {};
  ...
  }, [draft.profiles_data, draft.temporary_work_spouse_language, form, isSpouseProfile, profileId, draftSnap.isLoading]);

  const handleSave = async () => {
    const formData = form.getValues();
    let result;
    if (profileId && isSpouseProfile) {
      result = await draftStore.saveProfileSectionData(profileId, "language", formData);
    } else {
      result = await draftStore.saveSectionData("temporary_work_spouse_language", formData);
    }
  ...
  const onSubmit = async (data) => {
    ...
    if (profileId && isSpouseProfile) {
      ...
      await draftStore.markProfilePageComplete(profileId, `${visaType}/spouse-partner/language`);
    } else {
      await draftStore.saveSectionData("temporary_work_spouse_language", data);
  ...
  return (
    <Card className="rounded-2xl shadow-md bg-white">
      <CardHeader>
        <CardTitle className="text-2xl font-semibold">Spouse/Partner's Language</CardTitle>
        <p className="text-sm text-gray-600 mt-2">
          In this section, provide details about the spouse/partner&apos;s language proficiency.
        </p>
      </CardHeader>
      <CardContent>
