              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle>CRM Information</CardTitle>
                  </div>
                  <CardDescription>Synced from Zoho CRM</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  {userProfile?.zohoAccountName && (
                    <div>
                      <p className="text-sm font-medium text-gray-700 mb-1">Account</p>
                      <p className="text-gray-900">{userProfile.zohoAccountName}</p>
                    </div>
                  )}
                  
                  {userProfile?.mobile && (
                    <div>
                      <p className="text-sm font-medium text-gray-700 mb-1">Mobile</p>
                      <p className="text-gray-900">{userProfile.mobile}</p>
                    </div>
                  )}
                  
                  {(userProfile?.mailingStreet || userProfile?.mailingCity) && (
                    <div>
                      ...
                      <div className="text-gray-900 space-y-1">
                        {userProfile?.mailingStreet && <p>{userProfile.mailingStreet}</p>}
                        <p>
                          {[
                            userProfile?.mailingCity,
                            userProfile?.mailingState,
                            userProfile?.mailingZip
                          ].filter(Boolean).join(', ')}
                        </p>
                        {userProfile?.mailingCountry && <p>{userProfile.mailingCountry}</p>}
                      </div>
                    </div>
                  )}
                  ...
                  {userProfile?.zohoLastSyncedAt && (
                    <div className="pt-4 border-t">
                      <p className="text-xs text-gray-500">
                        Last synced: {new Date(userProfile.zohoLastSyncedAt).toLocaleString()}
                      </p>
                    </div>
                  )}
                </CardContent>
