...
function LanguageDialog({ editingRow, onSave, onCancel }) {
  ...
  const dialogForm = useForm({
    ...
    defaultValues: editingRow
      ? {
          ...editingRow,
          // migrate capitalised legacy values
          is_main_language: editingRow.is_main_language === "Yes" ? "yes"
            : editingRow.is_main_language === "No" ? "no"
            : editingRow.is_main_language || "no",
        }
      : {
          language: "",
          proficiency: "",
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <Input id="location" {...dialogForm.register("location")} data-testid="input-location" />
      ...
      <div>
        ...
        <Input id="reference_number" {...dialogForm.register("reference_number")} data-testid="input-reference-number" />
      ...
      <div>
        ...
        <Input id="overall_score" {...dialogForm.register("overall_score")} data-testid="input-overall-score" />
      ...
      <div className="border-t pt-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input id="listening" {...dialogForm.register("listening")} data-testid="input-listening" />
          </div>
          <div>
            <Label htmlFor="reading" className="mb-2 block text-sm">Reading</Label>
            <Input id="reading" {...dialogForm.register("reading")} data-testid="input-reading" />
          </div>
          <div>
            <Label htmlFor="writing" className="mb-2 block text-sm">Writing</Label>
            <Input id="writing" {...dialogForm.register("writing")} data-testid="input-writing" />
          </div>
          <div>
            <Label htmlFor="speaking" className="mb-2 block text-sm">Speaking</Label>
            <Input id="speaking" {...dialogForm.register("speaking")} data-testid="input-speaking" />
          </div>
        </div>
      </div>
...
export default function MainApplicantLanguagePage() {
  ...
  useEffect(() => {
    const sectionData = profileId
      ? draftSnap.draft?.profiles_data?.[profileId]?.language
      : draftStore.getSectionData('mainApplicant.language');

    if (!draftSnap.isLoading && sectionData && Object.keys(sectionData).length > 0) {
      // Migrate legacy is_english_main (Yes/No) → is_english_main_language (yes/no)
      const migratedEnglishMain =
        sectionData.is_english_main_language ||
        (sectionData.is_english_main === "Yes" ? "yes"
          : sectionData.is_english_main === "No" ? "no"
          : "no");

      form.reset({
        is_english_main_language: migratedEnglishMain,
        languages: sectionData.languages || [],
        has_english_test: sectionData.has_english_test || "no",
        english_tests: sectionData.english_tests || [],
        studied_in_english: sectionData.studied_in_english || "no",
        studied_in_english_details: sectionData.studied_in_english_details || "",
      }, { keepDefaultValues: true });
    }
  }, [draftSnap.isLoading, draftSnap.draft, profileId, form]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const result = profileId
        ...
        : await draftStore.saveSectionData("mainApplicant.language", data);

      if (result.success) {
        if (profileId) {
          await draftStore.markProfilePageComplete(profileId, 'partner/main-applicant/language');
        } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = profileId
        ? await draftStore.saveProfileSectionData(profileId, "language", currentData)
        : await draftStore.saveSectionData("mainApplicant.language", currentData);
      ...
      <Card className="rounded-2xl shadow-md bg-white">
        ...
        <CardContent>
          <form
            ...
            className="space-y-8"
          >
            {/* Question 1: Is the English language your main language? */}
            <div>
              <Label className="text-base font-medium mb-3 block">
            ...
            {/* Question 2: Have you undertaken any English language test within the last 36 months? */}
            <div>
              <Label className="text-base font-medium mb-3 block">
            ...
            {/* Question 3: Has the applicant studied in an English-medium institution? */}
            <div>
              <Label className="text-base font-medium mb-3 block">
