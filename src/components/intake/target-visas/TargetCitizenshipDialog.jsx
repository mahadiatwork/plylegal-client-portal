...
export function TargetCitizenshipDialog({ editingRow, onSave: onSubmit, onCancel, additionalMethods = [], reasonField = "reason_ceased" }) {
  const row = editingRow ? {
    ...editingRow,
    reason_ceased: editingRow.reason_ceased || editingRow.ceased_reason || "",
    ...Object.fromEntries(["date_obtained", "date_ceased"].flatMap((prefix) => {
      const day = String(editingRow[`${prefix}_day`] || "");
      const month = String(editingRow[`${prefix}_month`] || "");
      const months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
      return [
        [`${prefix}_day`, /^\d+$/.test(day) ? String(Number(day)) : day],
        [`${prefix}_month`, /^\d+$/.test(month) ? months[Number(month) - 1] || month : month],
      ];
    })),
  } : undefined;
  const reasonOptions = [...new Set([...CITIZENSHIP_REASON_OPTIONS, ...additionalMethods, ...(row?.how_obtained ? [row.how_obtained] : [])])];
  ...
  const handleFormSubmit = (data) => {
    onSubmit({
      ...editingRow,
      ...data,
      ...(reasonField === "ceased_reason" || editingRow?.ceased_reason !== undefined ? { ceased_reason: data.reason_ceased } : {}),
    });
  };
  ...
  for (const year of [row?.date_obtained_year, row?.date_ceased_year]) {
